...
cfg_select! {
    feature = "hot-reload" => {
        ...
        ///
        /// With the `hot-reload` feature, the module is reloaded whenever its
        /// bytecode file changes.
        ///
        /// See the [`stak`](https://docs.rs/stak) crate's documentation for full
        /// examples.
        #[macro_export]
    ...
    _ => {
        ...
        macro_rules! include_module {
            ($path:literal) => {
                $crate::UniversalModule::Static($crate::StaticModule::new(include_bytes!(
                    concat!(env!("OUT_DIR"), "/src/", $path)
                )))
            };
        }
    }
